...
export const ContractWriteModal = ({
  ...
  const { contractConnection, getWriteConnection, metamask } =
  ...
  const onConfirm = async () => {
    ...
    if (!writeConn) {
      setError(
        "Could not get a write connection to the contract on this network"
      );
      return;
    }
    setInProgress(true);
    ...
    <>
      ...
      <Modal isOpen={isOpen} onClose={close}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          {!inProgress && !txnHash && !error && (
            <ModalBody>{modalBody}</ModalBody>
          )}
          {error && (
            <ModalBody>Error: {error} Check the console for details.</ModalBody>
          )}
          {!error && txnHash && (
            <ModalBody>
