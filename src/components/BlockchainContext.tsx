...
export const BlockchainContext = ({ child }: { child: JSX.Element }) => {
  ...
  const [contractConnection, setContractConnection] =
    React.useState<ethers.Contract>();
  const [writeConnection, setWriteConnection] =
    React.useState<ethers.Contract>();
  ...
  const [tokenInfo, setTokenInfo] = React.useState<Map<number, TokenInfo>>(
    new Map()
  );
  ...
  const setUpReadConnection = () => {
    ...
    if (
      ...
      (!contractConnection ||
        (contractConnection.provider as any).network.chainId !==
          Number(chainId))
    ) {
      const contractInfo = CONTRACT_INFO[chainId];
      let provider;
  ...
  React.useEffect(() => {
    ...
    if (metamask) {
      ...
      setChainId("0x4"); //Default to Rinkeby
    }
  }, []);
